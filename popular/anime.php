...
$result = mysql_query("SELECT * FROM Popular_anime WHERE Page='".(($page+30)/30)."'") or die("Cache error: Unable to check the records!");
...
if ($page >= 0 && $time!= (date("j").(($page+30)/30))){
    ...
    $Mal = url("http://myanimelist.net/topanime.php?type=bypopularity&limit=".$page);
    ...
    if (strpos($Mal,'No anime titles found') !== false) {
        ...
        for ($i = 1; $i <= 30; $i++) {
            ...
            $episodes = parse($cell,'<div class="spaceit_pad">'.$type.', ',' eps,');
    ...
    if ($record == ""){
        ...
        mysql_query("INSERT INTO Popular_anime VALUES ('".date("j").(($page+30)/30)."','".(($page+30)/30)."','".$totaljson."')") or die('');
    }elseif (strpos($totaljson,'[{"id":"","title":"","type":"","image_url_short":"","image_url":""}]') !== false){
        header('Location: http://atarashii.yzi.me/popular/anime/'.(($page+30)/30));
    }else{
        mysql_query("UPDATE Popular_anime SET Time='".date("j").(($page+30)/30)."' WHERE Time='".$time."'") or die("");
        mysql_query("UPDATE Popular_anime SET Record='".$totaljson."' WHERE Record='".$record."'") or die("");  
    }
    mysql_close();
