...
$result = mysql_query("SELECT * FROM Top_manga WHERE Page='".(($page+30)/30)."'") or die("Cache error: Unable to check the records!");
...
if ($page >= 0 && $time!= (date("j").(($page+30)/30))){
    ...
    $Mal = url("http://myanimelist.net/topmanga.php?type=&limit=".$page);
    ...
    if ($record == ""){
        ...
        mysql_query("INSERT INTO Top_manga VALUES ('".date("j").(($page+30)/30)."','".(($page+30)/30)."','".$totaljson."')") or die('');
    }elseif (strpos($totaljson,'[{"id":"","title":"","type":"","image_url_short":"","image_url":""}]') !== false){
        header('Location: http://atarashii.yzi.me/top/manga/'.(($page+30)/30));
    }else{
        mysql_query("UPDATE Top_manga SET Time='".date("j").(($page+30)/30)."' WHERE Time='".$time."'") or die("");
        mysql_query("UPDATE Top_manga SET Record='".$totaljson."' WHERE Record='".$record."'") or die("");  
    }
    mysql_close();
