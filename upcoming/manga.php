...
$result = mysql_query("SELECT * FROM Upcoming_manga WHERE Page='".(($page+20)/20)."'") or die("Cache error: Unable to check the records!");
...
if ($page >= 0 && $time!= (date("j").(($page+20)/20))){
    ...
    $Mal = url("http://myanimelist.net/manga.php?sd=".date("j")."&sm=".date("n")."&sy=".date("y")."&em=0&ed=0&ey=0&o=2&w=&c[0]=a&c[1]=d&cv=1&q=&tag=&show=".$page);
    ...
    if ($record == ""){
        ...
        mysql_query("INSERT INTO Upcoming_manga VALUES ('".date("j").(($page+20)/20)."','".(($page+20)/20)."','".$totaljson."')") or die("");
    }elseif (strpos($totaljson,'[{"id":"","title":"","type":"","image_url_short":"","image_url":""}]') !== false){
        header('Location: http://atarashii.yzi.me/upcoming/manga/'.(($page+20)/20));
    }else{
        mysql_query("UPDATE Upcoming_manga SET Time='".date("j").(($page+20)/20)."' WHERE Time='".$time."'") or die("");
        mysql_query("UPDATE Upcoming_manga SET Record='".$totaljson."' WHERE Record='".$record."'") or die("");  
    }
    mysql_close();
